...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="w-full h-24 bg-navbar fixed top-0 left-0 right-0 z-40 border-b border-white/5">
        <div className="max-w-[1440px] mx-auto w-full h-full flex items-center justify-between px-6 md:px-12 relative">
          {/* 1. Logo */}
          ...
          <div className="hidden min-[1200px]:flex items-center gap-8 absolute left-1/2 -translate-x-1/2">
            {NAV_ITEMS.map((item) => (
              <div key={item} className="flex items-center gap-1 cursor-pointer group">
                <span className="text-gray-100 font-medium text-app-base whitespace-nowrap group-hover:text-primary transition-colors">
                  {item}
                </span>
